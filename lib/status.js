...
StatusController.prototype.show = function (req, res) {
  ...
  switch (option) {
    ...
    case 'getPeerInfo':
      ...
      break;
    case 'getActiveNodes':
      this.getActiveNodes(function (err, result) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
        res.jsonp(result);
...
StatusController.prototype.getActiveNodes = function (callback) {
  this.node.services.bitcoind.getActiveNodes(function (err, response) {
    if (err) {
      return callback(err);
    }
    var safenodes = response.result;
    callback(null, safenodes);
  });
};
